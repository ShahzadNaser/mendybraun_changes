...
(function() {
	...
	frappe.views.KanbanBoard = function(opts) {
		...
		function init() {
			get_fields_data().then(function(cards_data) {
				opts.cards = cards_data;
		...
		function get_fields_data() {
			return new Promise(function(resolve) {
				frappe.call({
					method: "mendybraun_changes.custom.kanban_board_custom.get_fields_data",
					args: {
						data: opts.cards,
						board: opts.board
					},
	...
	function prepare_card(card, state, doc) {
		...
		var fields = [];
		if(state.board && state.board.fields){
			$.each(state.board.fields || [], function(i,v) {
				var field_meta = frappe.meta.get_docfield(state.doctype, v.field, card.name)
				fields.push({ "label": field_meta.label, "value": card[v.field] });
			});
		}
